...
if os.getenv("DELTACHAT_TEST_INCUS"):
    for _key in ("REMOTE1", "REMOTE2"):
        _val = os.getenv(_key)
        if _val:
            _incus_remotes[_key] = _val

load_dotenv()

for _key, _val in _incus_remotes.items():
...
def _ensure_secure_join(rpc, inviter, joiner) -> None:
    ...
    try:
        with rpc:
            ...
            if success:
                ...
                # TEST #3: Secure Join
                # ==========================================
                if should_run(3) or should_run(4) or should_run(5) or should_run(6) or should_run(8) or should_run(9) or should_run(14) or should_run(23):
                    def _t3():
                        if not cool:
                            print("\n" + "="*50)
                ...
                if should_run(8):
                    def _t8():
                        ...
                        _ensure_secure_join(rpc, acc1, acc3)
                ...
                if should_run(14):
                    def _t14():
                        test_14_purge_messages.run(rpc, dc, acc1, acc2, remote1, remote2)
                        if not cool:
